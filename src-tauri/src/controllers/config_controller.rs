...
pub fn load_from_user_config() -> Option<BackupConfig> {
...
pub fn load_from_sample() -> Option<BackupConfig> {
...
pub fn set_backup_config(new_config: &BackupConfig) -> Result<(), Box<dyn std::error::Error>> {
    let user_config_path = get_user_config_path();
        ...
        if user_config_path.exists() {
            if let Ok(metadata) = fs::metadata(&user_config_path) {
                ...
                if modified_time > last_modified_time {
                    ...
                    if let Some(new_config) = load_from_user_config() {
