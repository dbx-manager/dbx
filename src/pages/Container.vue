...
import ContainerWithStatus from "../Component/containerWithStatus.vue";
import SettingsPanel from "../Component/SettingsPanel.vue";
defineProps();
  ...
  <v-container class="max-w-full! space-y-3 ">
    <div class="w-full flex flex-col md:flex-row justify-between gap-4">
      ...
      <div class="w-full transition-opacity" v-if="settignsVisability" >
        <settings-panel class="w-fit absolute right-13 mt-10 "/>
      </div>
      <v-btn @Click="settignsVisability?settignsVisability=false:settignsVisability=true" variant="text" ><v-icon>mdi-cog</v-icon></v-btn>
    </div>
